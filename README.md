...
<img src="https://github.com/Miklakapi/go-mqtt-tester/blob/master/README_IMAGES/json.png" />
<img src="https://github.com/Miklakapi/go-mqtt-tester/blob/master/README_IMAGES/home_assistant.png" />
...
3. On startup:
    - all files are read
    - MQTT Discovery config is published
    - current state is published
4. On file change:
    - configuration is re-published
    - state is updated immediately
5. On file removal:
    - MQTT discovery config is cleared
    - state topic is cleared
6. On Home Assistant side:
    - entities appear and disappear automatically via MQTT Discovery
...
- listens for create / modify / delete events

### Controller

- connects file system events with MQTT publishing
- validates JSON input
- enriches configs with Home Assistant device metadata
- manages lifecycle of entities (add / update / remove)
...
- `unique_id`, `state_topic` and `device` are generated automatically
- filename (without `.json`) is used as the entity identifier
